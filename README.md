...
変換処理では、制御文字・SO/SI 不正ペア・変換不能コード・コードページ未定義文字など、テキストエディターや Git での編集時に不具合を引き起こす可能性のある HEX コードを不正コードとして検知し、Markdown 形式のレポートファイルに詳細を出力します。
...
- **不正文字検知**: 制御文字（`x'00'`〜`x'3F'` 範囲）・SO/SI 不正ペア・変換不能コード（`x'FF'`）・コードページ未定義文字（`x'40'`〜`x'FE'` の範囲でコードページ上に定義のないバイト、DBCS 区間を除く）を検知し、レポートに記録。変換出力は `?` に置換して継続。
